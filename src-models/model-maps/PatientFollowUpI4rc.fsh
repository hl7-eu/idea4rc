...
* statusOfPatientAtLastFollow-up 1..* CodeableConcept "Status of patient at last follow-up (M)" """Describes the status at last follow-up 
...
* statusOfPatientAtLastFollow-up insert ObligationActorAndCode($actor-headneck, #SHALL:handle)
* statusOfPatientAtLastFollow-up insert ObligationActorAndCode($actor-sarcoma-i4rc, #SHALL:handle)
* statusOfPatientAtLastFollow-up ^comment = """Alive, No Evidence of Disease (NED) - 2000100071
...
* statusOfDiseaseAtLastFollow-up 1..* EpisodeEvent "Status of disease at last follow-up (M)" """Describes the disease status at the last follow-up. This value is redundant as it could be derived from the disease status in EpisodeEvent. If a Patient has 3 follow-ups, but the disease status (e.g., stable disease) did not change during those, the follow-ups will be grouped into a single one with the last date of the three of them. 
"""
* statusOfDiseaseAtLastFollow-up insert ObligationActorAndCode($actor-headneck, #SHALL:handle)
* statusOfDiseaseAtLastFollow-up insert ObligationActorAndCode($actor-sarcoma-i4rc, #SHALL:handle)
* statusOfDiseaseAtLastFollow-up ^comment = """Complete Remission - 32946
